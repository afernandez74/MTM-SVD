...
for key, value in CESM_LME.items():
    ...
    if key == 'ALL_FORCING': # 'ALL_FORCING' contains ensemble mean, which we don't want in the mean calculation
        ...
        means_filtered = butter_lowpass(
            means[key].isel(year=slice(filter_per,-filter_per)).TS, cutoff_frequency = 1/filter_per, sampling_frequency = 1)

        means_smooth[key] = xr.Dataset({'TS': (['year'], means_filtered)},
...
p1 = means[case].isel(year=slice(0,-1)).TS.plot(
        linestyle = '-',
        linewidth=1,
        zorder = 2,
        color = 'darkblue',
        alpha = 0.3,
        label = case)
p2 = means_smooth[case].isel(year=slice(0,-1)).TS.plot(
        linestyle = '-',
        linewidth=2,
        zorder = 5,
        color = 'darkblue',
